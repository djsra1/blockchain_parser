...
BTC_USD=pd.read_csv("./blockchain-parser/Courses/btc_usd.csv",sep=';',header = 0,  dtype = {'date': str,'value': float})
USD_RUB=pd.read_csv("./blockchain-parser/Courses/usd_rub.csv",sep=';',header = 0,  dtype = {'date': str,'value': float})
...
def get_BTC_addr_info(num,your_btc_address):
    ...
    df_detail_excel.to_excel('./blockchain-parser/Details/'+your_btc_address+'.xlsx', index=True)
...
df_aggregate_excel.to_excel('./blockchain-parser/index.xlsx', index=False)